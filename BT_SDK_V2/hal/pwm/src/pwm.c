...
uint8_t pwm_mode_switch_en[MOTOR_NUMBER];	//PWM??????
uint8_t pwm_loop_switch_mode[MOTOR_NUMBER];	//PWM??????
...
uint16_t m1_pwm_switch_timeout = 0; 	//PWM??????
uint32_t m1_old_time_flay = 0; 		//PWM??????
#endif
...
uint16_t m2_pwm_switch_timeout = 0; 	//PWM??????
uint32_t m2_old_time_flay = 0; 		//PWM??????
#endif
...
uint16_t m3_pwm_switch_timeout = 0; 	//PWM??????
uint32_t m3_old_time_flay = 0; 		//PWM??????
#endif

#if	CHARGE_ENABLE
#define CHARGE_PWM_GPIO		GPIO_PA3
#define CHARGE_TIME_NUM		TIM4
uint16_t charge_pwm_loop_swtich_mode = 0; 		//PWM??????
uint8_t charge_pwm_mode_switch_en	= 0; 		//PWM??????
uint16_t charge_pwm_switch_timeout 	= 0; 		//PWM??????
uint32_t charge_old_time_flay 		= 0; 		//PWM??????
...
{
	pwm_mode_flay_init();			//???PWM????
	pwm_mode_switch_en_init();		//???PWM??????
	pwm_loop_switch_mode_init();	//???PWM??????
	pwm_work_en_flay_init();		//???PWM??????
	
	#if	M1_ENABLE
		old_m1_pwm_mode_flay = 0; 	//PWM?????
		m1_pwm_switch_timeout = 0; 	//PWM??????
		m1_old_time_flay = 0; 		//PWM??????
	#endif
	
	#if	M2_ENABLE
		old_m2_pwm_mode_flay = 0; 	//PWM?????
		m2_pwm_switch_timeout = 0; 	//PWM??????
		m2_old_time_flay = 0; 		//PWM??????
	#endif
	
	#if	M3_ENABLE
		old_m3_pwm_mode_flay = 0; 	//PWM?????
		m3_pwm_switch_timeout = 0; 	//PWM??????
		m3_old_time_flay = 0; 		//PWM??????
	#endif
}
...
void user_pwm_timer_init(void)
{
	...
	timer_pcl_reload(PWM_MODE_TIMEOUT,1599,9);
